...
const createInterestsEl = (parent, title, photo) => {
  const wrapper = c("div");
  const img = c("img");
  const interestsTitle = c("p");

  wrapper.className = "interestsCard";
  img.setAttribute("alt", title);
  img.setAttribute("src", photo);
  interestsTitle.textContent = title;

  wrapper.append(img, interestsTitle);
  parent.appendChild(wrapper);
};

export { c, q, createCard, createFriendEl, createMessageEl, createInterestsEl };
